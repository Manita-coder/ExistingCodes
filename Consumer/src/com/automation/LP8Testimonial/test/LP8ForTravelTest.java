...
public class LP8ForTravelTest {
	...
	String defaultCodeForLP8Travel = "BAACOJ";

	/*
	 * When running lp7areviews, Travel cards need to go to lp7linkpath to modify
	 * Path (Line 25)
	 */
...
//		Config for Tablet
	...
	public void contentTesting() throws IOException, WriteException, BiffException, InterruptedException {
		...
		String inputFile = "target/LP8b_Travel_Tab_Result.xls";
		...
		Sheet sheet1 = workbook1.getSheet(3);
		...
		for (int row = 8; row <sheet1.getRows(); row++) {
			...
			testForHead(sheet1, 8, row, "travelRewards", excelSheet, row, column);// cashRewards, travelRewards,
																					// bbr,visa
...
//		--------------Deepthy--------------------
	...
	private void testForHeadDefaultCode(Sheet sheetReference, int rowReference, int columnReference, String text,
		...
		String URL_CashRewards = format + defaultCodeForLP8Travel;
		String URL_TermsandConditionsCashRewards = format + defaultCodeForLP8Travel + "&pid=dppf";
		String URL = page8 + "?code=" + defaultCodeForLP8Travel;
		driver.get(URL);
		Thread.sleep(3000);
		if (lp7linkpath.isAvaiblableBigButton(text)) {
			String ButtonBig = lp7linkpath.gettingBigButtonText(text);
			...
			addcolor(sheet, column + 1, row - 7, URL_CashRewards);
			addcolor6(sheet, column + 3, row - 7, "Not Find Buttom");
		}

		if (lp7linkpath.isAvaiblableHeadTC(text)) {
			String TermsBig = lp7linkpath.gettingBigCTText(text);
