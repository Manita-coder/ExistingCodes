...
public class LP8ForCashTest {
	...
	//String defaultCodeForLP8CashRewards = "BAACOJ";
...
//	final String page8 = "https://promo.bankofamerica.com/ccsearchlp8/card-finder/";
	final String page8 = "https://starcom_1:Q4makesME@qa-promo-boa.groupeconnect.io/ccsearchlp82/";

	final String format = "https://secure.bankofamerica.com/applynow/initialize-workflow.go?requesttype=C&campaignid=";
	final String formatT_C = "https://www.bankofamerica.com/credit-cards/terms-and-conditions/";
...
//	driver.manage().window().setSize(new Dimension(768, 1024));
	...
	{
		String inputFile = "target/LP8_Cash_Result.xls";
		...
		// System.out.println("No of rows present in excel "+noOfRows);
		...
		for (int row = 8; row < sheet1.getRows(); row++) 
		{
			...
			if (!expectedCode.trim().equalsIgnoreCase("")) 
			{
				...
				testForHead(sheet1, 8, row, "cashRewards", excelSheet, row, column);// cashRewards, travelRewards,
																					// bbr,visa
...
//				generalcards.testForCallToApplyByPhoneNo(sheet1, row, excelSheet, row, column);
	...
	private void testForHead(Sheet sheetReference, int rowReference, int columnReference, String text,
		...
		/*
		 * if (Code.isEmpty()) { Code = defaultCodeForLP8CashRewards; }
		 */
		// System.out.println(Code);
//		--------------Deepthy--------------------
	...
	/*private void testForHeadDefaultCode(Sheet sheetReference, int rowReference, int columnReference, String text,
		...
		String URL_CashRewards = format + defaultCodeForLP8CashRewards;
		String URL_TermsandConditionsCashRewards = format + defaultCodeForLP8CashRewards + "&pid=dppf";
		String URL = page8 + "?code=" + defaultCodeForLP8CashRewards;
		driver.get(URL);
		Thread.sleep(3000);
