...
public class LP8ForNonRewardsTest {
...
//		Config for Tablet
	...
	public void contentTesting() throws IOException, WriteException, BiffException, InterruptedException {
		String inputFile = "target/LP8_NonRewards_Tab_Result.xls";
		...
		for (int row = 8; row <sheet1.getRows(); row++) {
			...
			testForHead(sheet1, 8, row, "bbr", excelSheet, row, column);// cashRewards, travelRewards, bbr,visa
	...
	private void testForHead(Sheet sheetReference, int rowReference, int columnReference, String text,
		...
		String URL_CashRewards = expectedAppLink;
//		String URL_CashRewards= format+Code;			

		String URL_TermsandConditionsCashRewards = format + Code + "&pid=dppf";
	...
	private void testForHeadDefaultCode(Sheet sheetReference, int rowReference, int columnReference, String text,
		...
		String URL_CashRewards = format + DefaultCodeForLP7BBR;
		String URL_TermsandConditionsCashRewards = format + DefaultCodeForLP7BBR + "&pid=dppf";
		String URL = page8 + "?code=" + DefaultCodeForLP7BBR;
		driver.get(URL);
		Thread.sleep(3000);
